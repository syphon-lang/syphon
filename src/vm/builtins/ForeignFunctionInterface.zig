...
export fn evaluateCallback(cif: [*c]ffi.ffi_cif, maybe_return_address: ?*anyopaque, arguments: [*c]?*anyopaque, maybe_data: ?*anyopaque) void {
    ...
    var args = std.ArrayList(Code.Value).init(vm.allocator);
    ...
    for (arguments[0..cif.*.nargs], 0..) |argument, i| {
        switch (cif.*.arg_types[i].*.type) {
            ...
            ffi.FFI_TYPE_UINT8 => {
                ...
                args.append(Code.Value{ .int = @intCast(value) }) catch evaluateCallbackFailed();
            },
            ...
            ffi.FFI_TYPE_SINT64 => {
                ...
                args.append(Code.Value{ .int = value }) catch evaluateCallbackFailed();
            },

            ffi.FFI_TYPE_FLOAT => {
                const value = @as(*f32, @ptrCast(@alignCast(argument.?))).*;
                args.append(Code.Value{ .float = @floatCast(value) }) catch evaluateCallbackFailed();
            },
            ...
            ffi.FFI_TYPE_POINTER => {
                ...
                args.append(Code.Value{ .int = @bitCast(value) }) catch evaluateCallbackFailed();
            },
    ...
    const return_value = function.call(vm, args.items);
